...
class CrdtSchemaTableRepository {
  ...
  /// Emits [CrdtSchemaTable]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `CrdtSchemaTable.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<CrdtSchemaTable>> watch(
    _isd.DatabaseSession session, {
    _isd.WhereExpressionBuilder<CrdtSchemaTableTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<CrdtSchemaTable>(
      where: where?.call(CrdtSchemaTable.t),
      orderBy: orderBy?.call(CrdtSchemaTable.t),
      orderByList: orderByList?.call(CrdtSchemaTable.t),
      limit: limit,
