...
class CrdtSchemaColumnRepository {
  ...
  /// Emits [CrdtSchemaColumn]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `CrdtSchemaColumn.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<CrdtSchemaColumn>> watch(
    _isd.DatabaseSession session, {
    _isd.WhereExpressionBuilder<CrdtSchemaColumnTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<CrdtSchemaColumn>(
      where: where?.call(CrdtSchemaColumn.t),
      orderBy: orderBy?.call(CrdtSchemaColumn.t),
      orderByList: orderByList?.call(CrdtSchemaColumn.t),
      limit: limit,
