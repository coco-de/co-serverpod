...
| `lib/src/sync/engine.dart`·`space_state.dart` | 미전송 건수의 근거(unibook#14183): 기기(follower)가 서버 핸드셰이크의 자기 노드 체크포인트로 (space, 자기 노드) `offline_sync_space_nodes.lastReceivedHlc` 를 **덮어쓰고**(A), `once` 세션에서 서버 `OfflineSyncClose` 를 받은 뒤에만 보낸 변경의 최대값을 기록한다(B). 동기화 루프에 기기 쪽 DB 쓰기가 늘었고, 와이어 프로토콜·스키마·권위 피어 동작은 무변경. `countUnsentRows`(수집 필터를 자기 노드로 좁힌 3쿼리), `OfflineSyncSpaceState.checkpointOf`·`handshakenSpaceIds` ([동기화 상태](#동기화-상태와-미전송-건수)) |
| `lib/src/database/database.dart`·`recorder.dart` | `unsentRowCount`·`watchUnsentRowCount`·`watchUnsentRowCountTriggers`, `@internal replaceSyncCheckpoint`(단조 증가가 아닌 덮어쓰기) |
...
| `serverpod_offline_sync_client` `lib/src/sync/sync_status.dart` (신규)·`pubspec.yaml` | 상태 API `OfflineSyncStatusTracker`·`OfflineSyncStatus`·`OfflineSyncPhase` (unibook#14183), `clock` 의존 추가 |
...
- **적게 세지 않고, 많이 셀 수는 있습니다.** 서버가 거부한 회차(K2 등)와 기기 쪽 실패는 ②에 닿지 않아 줄지
  ...
  까지 많게 셉니다. 서버가 이 사용자와 더 동기화하지 않는 space 의 자기 행은 계속 셉니다.
...
- **비용**: 행·필드·tombstone 3쿼리의 행 id 합집합입니다. 데스크톱 SQLite 실측(2026-09-24, 이 구현):
  1천 행 8–16ms·1만 행 49–56ms(확인값 없음, 전량), 확인값 이후 10% 갱신 시 1천 행 3–5ms·1만 행 13–16ms.
  모바일은 측정하지 않았습니다. 행에 필드 조건을 `any` 로 거는 한 쿼리는 설계 프로브에서 1만 행 1.3–2.4초여서
  쓰지 않습니다.

### 서버 연속 동기화 간격

`initializeOfflineSync(continuousSyncInterval:)` 이 연속 동기화 회차 사이 대기(기본 200ms, 업스트림과 같음)를
정합니다. 유휴 회차 주기는 대략 유휴 타임아웃 1초 + 이 간격입니다. 기기 쪽은
`OfflineSyncDatabaseSession.wraps(continuousSyncInterval:)` 로 따로 정합니다.
...
간격은 서버 전체에 하나입니다. 화면마다(세션마다) 다른 간격은 아직 없습니다.
