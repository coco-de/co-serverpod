...
- **서버 노드는 space 마다 하나** (unibook#14218): 업스트림은 DB 하나의 모든 space 가 **노드 하나를 공유**했습니다.
  ...
  **영속 사용자 없이 연 DB(서버)** 에서 space 마다 노드를 줍니다(`managers/space.dart`). 기기(영속 사용자로 연 DB)
  는 지금처럼 설치 단위 노드 하나를 모든 space 가 공유합니다. 설정으로 끌 수 없습니다 — DB 를 여는 방식이
  정합니다. 이미 노드를 공유하던 space 는 다음 사용 때 새 노드를 받고, 그 시계는
  max(공유 노드 `lastHlc`, 그 space 에 저장된 최대 스탬프)에서 시작합니다(기기 쪽 `_preserveLatestCurrentNodeHlc`
  와 대칭). 한 세션이 여러 space 를 다루면 서버 노드도 여럿이지만 체크포인트가 (space, 노드) 단위라 정합이
  유지됩니다(`space_node_test.dart`) — 다만 한 space 의 K1 은 그 세션 전체를 멈춥니다.
